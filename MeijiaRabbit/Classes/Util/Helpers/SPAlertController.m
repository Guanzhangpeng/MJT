...
#define SP_LINE_COLOR \
({UIColor *normalColor = [[UIColor darkGrayColor] colorWithAlphaComponent:0.3];\
if (@available(iOS 13,*)) {\
    normalColor = [UIColor colorNamed:@"ColorBg"];\
}(normalColor);})

#define SP_NORMAL_COLOR \
({UIColor *normalColor = [[UIColor whiteColor] colorWithAlphaComponent:0.7];\
if (@available(iOS 13,*)) {\
    normalColor = [UIColor colorNamed:@"ColorContent"];\
}(normalColor);})

#define SP_TITLE_COLOR \
({UIColor *normalColor = [UIColor blackColor];\
if (@available(iOS 13,*)) {\
    normalColor = [UIColor colorNamed:@"LblColor"];\
}(normalColor);})

#define SP_SELECTED_COLOR \
({UIColor *normalColor = [UIColor colorWithWhite:1 alpha:0.4];\
if (@available(iOS 13,*)) {\
    normalColor = [UIColor colorNamed:@"ColorSelected"];\
}(normalColor);})
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    action.titleColor = SP_TITLE_COLOR;
...
- (instancetype)initWithTitle:(nullable NSString *)title style:(SPAlertActionStyle)style handler:(void (^ __nullable)(SPAlertAction *action))handler {
    ...
    if (style == SPAlertActionStyleDestructive) {
        ...
        self.titleColor = SP_TITLE_COLOR;
...
- (void)touchDown:(UIButton *)sender {
    ...
    if (alert.needDialogBlur) {
        ...
        sender.backgroundColor = SP_SELECTED_COLOR;//[[UIColor grayColor] colorWithAlphaComponent:0.1]; // 该颜色比'取消action'上的分割线的颜色浅一些
    }
}
