...
#pragma mark -  根据unix时间戳字符串 转换为时间字符串
+(NSString *)unixTimeStampToDateTimeStr:(NSString *)unixTimeStampStr format:(NSString *)format{
    
    if (!unixTimeStampStr) {
        return @"";
    }else if([unixTimeStampStr isEqualToString:@"0"]){
        return @"";
    }
    double unixTimeStamp = 0 ;
    if (unixTimeStampStr.length>11) {
        unixTimeStamp = [unixTimeStampStr doubleValue]/1000.0f;
    }else
    {
        unixTimeStamp = [unixTimeStampStr doubleValue];
    }
...
+ (NSTimeInterval )timeStringWithFormat:(NSString *)format time:(NSString *)time {
